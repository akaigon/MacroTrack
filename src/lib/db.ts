...
export interface Settings {
  ...
  fatRule: MacroRule;
  currentTargets: Macros; // the DEFAULT calorie + macro goals (the baseline)
  // Optional "macro cycling": different targets per weekday that repeat weekly.
  // Index 0 = Sunday … 6 = Saturday. A null entry means "use the default".
  weeklyEnabled?: boolean;
  weeklyTargets?: (Macros | null)[]; // length 7 when present
  theme?: "system" | "light" | "dark";
}

// dayTargets — a one-off macro override for a SPECIFIC calendar date. Takes
// priority over the weekly schedule and the default targets for that day only.
...
export class MacroTrackDB extends Dexie {
  ...
  dayTargets!: Table<DayTargets, string>;
  ...
  constructor() {
    ...
    // version(2) adds per-date macro overrides. Dexie keeps the version(1)
    // tables and just creates this new one, so existing data is preserved.
